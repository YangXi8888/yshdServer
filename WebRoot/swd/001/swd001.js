	...
	$("#dataTable").datagrid({
		...
		columns : [[{
			...
			formatter : function(value, row, index) {
				var btnPanel = "";
				btnPanel = "<a href='#' class='editcls'  style='text-decoration: none' onclick=downLoadFile('" + row.SCJL_ID + "')  href='#'>下载</a>";
				btnPanel += "&nbsp&nbsp;&nbsp&nbsp;&nbsp&nbsp;&nbsp&nbsp;<a class='editcls' onclick=deleteFile('" + row.SCJL_ID + "') href='#'>删除</a>";
				return btnPanel;
			}
		}]]
	});
		...
		success : function(responseText, textStatus, XMLHttpRequest) {
			if (checkResponse(responseText)) {
				for (var i = 0; i < responseText.data.list.length; i++) {
					$("#qyYhDm").append("<option value='" + responseText.data.list[i].QYYH_DM + "'>" + responseText.data.list[i].QYYH_MC + "</option>");
				}
			} else {
				// 判断是否超时
				if (!isTimeout(responseText)) {
					$.messager.alert(commomMessageTitle, responseText.msg, 'error', function() {
						queryData();
					});
				}
			}
		...
		if (r == true) {
			...
			$.ajax({
				...
				success : function(responseText, textStatus, XMLHttpRequest) {
					$.messager.progress('close');
					if (checkResponse(responseText)) {
						$.messager.alert(commomMessageTitle, responseText.msg, 'info', function() {
							queryData();
						});
					} else {
						// 判断是否超时
	...
	$.ajax({
		...
		success : function(responseText, textStatus, XMLHttpRequest) {
			...
			if (checkResponse(responseText)) {
				...
				$("#dataTable").datagrid("loadData", dataList.slice(0, 15));
				// 设置分页的相关属性
