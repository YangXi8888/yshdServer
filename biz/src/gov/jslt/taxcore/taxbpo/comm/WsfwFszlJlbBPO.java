...
import gov.jslt.taxevent.comm.WsfwFszlJlbVO;

public class WsfwFszlJlbBPO extends CssBaseBPO {

public WsfwFszlJlbBPO() {
super();
}

private static final String TABLENAME = "T_WSFW_FBZLJLB";

private static  WsfwFszlJlbVO loadConcreteBPO(ResultSet rs) throws SQLException {
 		WsfwFszlJlbVO    vo = new WsfwFszlJlbVO()	 ;
     	vo.setBz(rs.getString("BZ"));
     	vo.setFbzldm(rs.getString("FBZL_DM"));
     	vo.setFbzlmc(rs.getString("FBZL_MC"));
     	vo.setLsh(rs.getString("LSH"));
     	vo.setSfbb(rs.getString("SFBB"));
     	vo.setSfgg(rs.getString("SFGG"));
     	vo.setSwglm(String.valueOf(rs.getLong("SWGLM")));
     	vo.setXh(rs.getString("XH"));
     	vo.setZlly(rs.getString("ZLLY"));
...
private static WsfwFszlJlbVO queryByWhere(Connection con,String tablename,String sqlOrder,String sqlWhere,ArrayList sqlParams,boolean isCount) throws SQLException {

	ResultSet result=null;
	PreparedStatement ps=null;
	result = findAllByWhere(con,tablename,sqlOrder,  sqlWhere, sqlParams,isCount,result,ps);

	WsfwFszlJlbVO vo = null;
	if (result != null) {
		    if(result.next())
...
public static boolean insert(Connection con , WsfwFszlJlbVO vo)throws SQLException {
...
* @throws SQLException
*/
public static boolean deleteByPK(Connection con,String LSH, String XH) throws SQLException {
	String strSqlWhere="LSH=?  AND XH=? ";
...
* @throws SQLException
*/
public static WsfwFszlJlbVO queryByPK(Connection con,String LSH, String XH) throws SQLException {
	String strSqlWhere="LSH=?  AND XH=? ";
	ArrayList sqlParams=new ArrayList();
 	sqlParams.add(LSH);
 	sqlParams.add(XH);
...
* @throws SQLException
*/
 public static boolean updateByPK(Connection con,String LSH, String XH, WsfwFszlJlbVO vo) throws SQLException {
	String strSqlWhere="LSH=?  AND XH=? ";
	ArrayList sqlParams=new ArrayList();
 	sqlParams.add(LSH);
 	sqlParams.add(XH);
