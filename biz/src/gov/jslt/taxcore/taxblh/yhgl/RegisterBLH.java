...
public class RegisterBLH extends BaseBizLogicHandler {
	...
	private ResponseEvent getYzm(RequestEvent reqEvent, Connection conn)
		...
		if (result > 0) {
			resEvent.setRepCode("ZB0032");
			resEvent.setReponseMesg(ZBCoreHelper.getJyztMc(conn, "ZB0032"));
			return resEvent;
		}

		// 3.发送验证码
		JsonResData jsonResData = DxTools.sendYzm(conn, sjHm, sjYzm,
				DxTools.getYzmMb("注册自然人实名办税平台", sjYzm, "10"));
	...
	private ResponseEvent submitData(RequestEvent reqEvent, Connection conn)
		...
		pstmt.execute();
		pstmt.close();
		resEvent.setReponseMesg(ZBCoreHelper.getJyztMc(conn, "ZB0033"));
		return resEvent;
	}
	...
	private int isRegister(String zjlxDm, String zjHm, String sjHm,
		...
		CachedRowSet rs = QueryBPO
				.findAll(
						conn,
						"SELECT UUID FROM T_ZB_LOGIN A WHERE ((A.SFZMLX_DM=? AND A.SFZHM=?) OR SJHM=?) AND XY_BJ='1'",
						sqlParams);
		if (rs.size() > 0) {
			return 1;
		} else {
