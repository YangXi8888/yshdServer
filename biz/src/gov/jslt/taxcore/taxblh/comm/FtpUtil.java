...
public class FtpUtil {
	...
	public static boolean uploadFile(String url, String username,
		...
		try {
			...
			if (!FTPReply.isPositiveCompletion(reply)) {
				...
				throw new TaxBaseBizException("连接ftp服务器失败!");
			}
			...
			if (false == wjj_flag) {
				if (false == creatDir(ftp, path)) {
					throw new TaxBaseBizException("目录" + path + "创建失败!");
				}
			}
			...
			if (ftp.isConnected()) {
				try {
					...
					throw new TaxBaseBizException(ioe.getMessage());
				}
			}
		}
		return success;
	...
	public static boolean creatDir(FTPClient ftp, String path)
			throws TaxBaseBizException {
		boolean flag = false;
